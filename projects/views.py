...
from .models import amazon_links_list
import json
...
flipkart_links = amazon_links_list.objects.create()
flipkart_links.product_name= product_names_clear
flipkart_links.projuct_link= links_done
flipkart_links.save()

flipkart_dic={key : value for key , value in zip(flipkart_links.product_name,flipkart_links.projuct_link)}

dict_project ={'product_name':flipkart_links.product_name,'product_link': flipkart_links.projuct_link}
product_name = flipkart_links.product_name
product_links = flipkart_links.projuct_link
def projects(request):
    return render(request , 'projects/projects.html',{"flipkart_dic":flipkart_dic})

# def product(request , pk):
#     productObj = 
#     return render(request, 'projects/single-link.html' , {'product':productObj})

# def link(request):
#     return render(request , 'projects/single-link.html' ,{""})